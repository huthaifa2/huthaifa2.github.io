  ...
  return (
    <div className="w-full bg-gradient-to-br from-white/90 to-white/60 dark:from-[#24242c]/90 dark:to-[#1a1a28]/80 backdrop-blur-xl border border-white/50 dark:border-white/10 rounded-[2.5rem] shadow-[0_8px_30px_rgb(0,0,0,0.06)] overflow-hidden transition-colors duration-500">
      <div className="w-full aspect-video bg-black overflow-hidden rounded-t-[2.5rem] relative group">
        ...
        />
        <button
          onClick={() => window.open('/images/figma/Figma.mp4', '_blank')}
      ...
      <div className="p-8 md:p-12">
        ...
        <div className="flex flex-wrap gap-2 mb-6">
          {['Figma', 'UI/UX Design', 'Prototyping', 'Interaction Design'].map((tool, i) => (
            <span key={i} className="px-4 py-1.5 bg-white dark:bg-white/10 text-[#1d1d1f] dark:text-white rounded-full text-sm font-semibold border border-gray-200 dark:border-white/5 shadow-sm transition-colors duration-500">
              {tool}
            </span>
          ))}
        </div>
        ...
        <p className="text-[#1d1d1f] dark:text-gray-300 text-sm md:text-lg opacity-90 leading-relaxed font-bold transition-colors duration-500">
          A walkthrough of interactive Figma prototypes I designed, showcasing advanced interaction patterns, smooth transitions, and polished micro-animations across multiple UI systems.
        </p>
      </div>
    </div>
  )
