...
import AjeerProject from './AjeerProject'
import PolarisProject from './PolarisProject'

export default function AppCard({ onOpenAjeer, onOpenPolaris }) {
    ...
    <div className="relative rounded-[3rem] overflow-hidden">
      {!revealed && (
        <div className="absolute inset-0 z-30 flex flex-col items-center justify-center bg-[#f5f5f7]/60 dark:bg-[#13131A]/70 backdrop-blur-sm">
          ...
          <h4 className="text-2xl md:text-3xl font-bold text-[#1d1d1f] dark:text-white mb-6 text-center px-4">
            I've transitioned to UI/UX, Graphic & Motion Design.
          </h4>
          <button
      ...
      <div className={`transition-all duration-700 ${!revealed ? 'grayscale opacity-60 pointer-events-none select-none' : ''}`}>
        <div className="relative">
          <div className={`space-y-24 overflow-hidden transition-all duration-1000 ease-in-out ${expanded ? 'max-h-[5000px]' : 'max-h-[650px]'}`}>
            <AjeerProject onOpenSlider={onOpenAjeer} />
            <PolarisProject onOpenSlider={onOpenPolaris} />
          </div>

          {!expanded && (
            <div className="absolute bottom-0 left-0 right-0 h-96 rounded-b-[3rem] overflow-hidden pointer-events-none transition-colors duration-300 z-10">
              <div
